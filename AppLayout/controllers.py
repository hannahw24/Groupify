...
def getUserProfile(userID=None):
    ...
    db.banner.update_or_insert(bannerOfWho=getIDFromUserTable(userID))
    bannerList = db(db.banner.bannerOfWho == getIDFromUserTable(userID)).select().as_list()
    ...
    bannerImgs = bannerList[0]["imgList"]
    ...
    return dict(session=session, editable=editable_profile(userID), friendsList=friendsList, topTracks=topTracks,
                topArtists=topArtists, imgList=imgList, trackLinks=trackLinks, artistLinks=artistLinks, bannerImgs=bannerImgs, profile_pic=profile_pic,
                )
...
# ------- WORKING------        
@action('banner/<userID>', method=["GET", "POST"])
@action.uses('banner.html', session)
def banner(userID=None):
    ...
    if form.accepted:
        if form.vars["Search"] == "":
            print ("empty input")
            return dict(form = form, session=session, editable=False, topAlbums=topAlbums, topArtists=topArtists,
            imgList=imgList, albumLinks=albumLinks, artistLinks=artistLinks, totalResults=totalResults, bannerImgs=bannerImgs)
        ...
        spotify = spotipy.Spotify(auth_manager=auth_manager)
        # Input what you want to see, see the spotipy API for parameters you can place to specify output
        results = spotify.search(form.vars["Search"], type='album', limit=5)
        print (results)
        stored = results
        ...
        imgList = biglist[2]
        print ("imgList ", imgList)
        albumLinks = biglist[3]
        print ("albumLinks", albumLinks)
        artistLinks = biglist[4]
        print ("artistLinks ", artistLinks)
        return dict(form = form, session=session, editable=False, topAlbums=topAlbums, topArtists=topArtists,
        imgList=imgList, albumLinks=albumLinks, artistLinks=artistLinks, totalResults=totalResults, bannerImgs=bannerImgs)
